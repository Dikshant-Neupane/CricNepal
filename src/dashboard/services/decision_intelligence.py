...
def format_recommendation_card(
    ...
    """
    Format recommendations as HTML card for dashboard display.
    ...
    Returns:
        HTML string for st.markdown with unsafe_allow_html=True
    """
    if not recommendations:
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="insight-box"><strong>Status:</strong> No recommendations available.</div>
            </div>
        </div>
        """
    ...
    for rec in recommendations:
        ...
        label = rec.get("label", "Recommendation")
        text = rec.get("text", "")
        priority = rec.get("priority", 0)
        
        # Map type to CSS class
        box_class = f"insight-box-{rec_type}" if rec_type != "info" else "insight-box"
        
        items_html += f"""
        <div class="{box_class}">
            <strong>#{priority} — {label}:</strong> {text}
        </div>
